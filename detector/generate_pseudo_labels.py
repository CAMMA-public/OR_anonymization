...
def parse_args():
    ...
    parser.add_argument(
        "--pred_path", default="/media/camma-monitor/Storage_postprocessing2/pose_detection/mva/009_SV_JPG_iter2_mva_tracking_reid.lmdb", nargs='+', help="path to detections"
    )
    ...
    parser.add_argument(
        "--root_dir", default="/media/camma-monitor/Storage_postprocessing3/009_SV_JPG", nargs='+', help="path to detections"
    )
    parser.add_argument(
        "--save_root_dir", default="009", nargs='+', help="path to detections"
    )
    parser.add_argument(
        '--kpt_thresh',
        type=float,
...
def main(args):
    ...
    for f_n in range(file_num):
        ...
        for c_id, sub_dir in enumerate(sub_dirs):
            os.makedirs(os.path.join(args.save_root_dir, sub_dir), exist_ok=True)
            ...
            for img_name in tqdm(img_names):
                ...
                for c_id, sub_dir in enumerate(sub_dirs):
                    ...
                    if boxes_data is not None:
                        ...
                        #     continue
                        image_data = {}
                        image_path = os.path.join(args.save_root_dir, sub_dir, img_name)
                        ori_img_path = os.path.join(root_dir, img_key)
