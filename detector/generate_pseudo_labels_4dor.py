...
def parse_args():
    ...
    parser.add_argument(
        "--pred_path", default="/media/camma-monitor/Storage_postprocessing2/pose_detection/mva/006_RS_JPG_iter1_mva_tracking_bi.lmdb", nargs='+', help="path to detections"
    )
...
def main(args):
    ...
    if isinstance(args.save_root_dir, str):
        save_root_dirs = [args.save_root_dir]
    else:
        save_root_dirs = args.save_root_dir
    ...
    for f_n in range(file_num):
        ...
        save_root_dir = save_root_dirs[f_n]
        
        img_dicts_pkl = os.path.join(root_dir, 'img_dicts.pkl')
        ...
        if visualize:
            ...
            save_dir = os.path.join('debug_reid', base_name)
        ...
        with env.begin() as txn:
            ...
            cursor = txn.cursor()
            count = count_seed
            for f_id in tqdm(range(frame_num)):
                count += 1
                ...
                for c_id in range(cam_num):
                    ...
                    if using_pose:
                        ...
                        key_boxes_scores = f"{img_key}/scores".encode('utf-8')
                    ...
                    if boxes_data is not None:
                        ...
                        if using_mva:
                            matching_scores = matching_scores[indices]
                        if using_pose:
                        ...
                        if using_mva:
                            new_matching_scores = matching_scores[indices]
                        if using_pose:
                            new_kpts_scores = kpts_scores[indices]
                            avg_kpts_scores = np.mean(new_kpts_scores, axis=1)
                        
                        if visualize:
                            img_path = os.path.join(root_dir, 'colorimage', img_name)
                            img = cv2.imread(img_path)
                            for i in range(len(new_boxes)):
                                x1 = int(new_boxes[i][0])
                                y1 = int(new_boxes[i][1])
                                x2 = int(new_boxes[i][2])
                                y2 = int(new_boxes[i][3])
                                box_score = round(new_boxes_score[i], 2)
                                # box_score = (y2 - y1) * (x2 - x1)
                                cv2.rectangle(img, [x1, y1], [x2, y2], color=(0, 255, 0), thickness=3)
                                if using_mva:
                                    if using_pose:
                                        kpts_score = round(avg_kpts_scores[i], 2)
                                        if new_matching_scores[i] != -1:
                                            matching_score = round(new_matching_scores[i], 2)
                                            score_txt = str(box_score) + ' K:' + str(kpts_score) + ' M:' + str(matching_score)
                                        else:
                                            score_txt = str(box_score) + ' K:' + str(kpts_score)
                                    else:
                                        if new_matching_scores[i] != -1:
                                            matching_score = round(new_matching_scores[i], 2)
                                            # if matching_score >= 0.2 or box_score >= 0.2:
                                            #     continue
                                            score_txt = str(box_score) + ' M:' + str(matching_score)
                                        else:
                                            # continue
                                            score_txt = str(box_score)
                                else:
                                    if using_pose:
                                        kpts_score = round(avg_kpts_scores[i], 2)
                                        score_txt = str(box_score) + ' K:' + str(kpts_score)
                                ...
                                cv2.putText(img, score_txt, [x1, y2], cv2.FONT_HERSHEY_PLAIN, 2, (0, 0, 255), thickness=3)
                        ...
                        #     continue
                        image_data = {}
                        image_path = os.path.join(save_root_dir, img_name)
                        ori_img_path = os.path.join(root_dir, 'colorimage', img_name)
                        shutil.copy(ori_img_path, image_path)
                        image_data['file_name'] = image_path
                        image_data["height"] = 1536
                        image_data["width"] = 2048
                        image_annotations = []
                        for i in range(len(new_boxes)):
                            x_min = int(new_boxes[i][0])
                            y_min = int(new_boxes[i][1])
                            w = int(new_boxes[i][2] - new_boxes[i][0])
                            h = int(new_boxes[i][3] - new_boxes[i][1])
                            info = {}
                            info["bbox"] = [x_min, y_min, w, h]
                            info["category_name"] = 'person'
                            image_annotations.append(info)
                        image_data["annotations"] = image_annotations
                        all_image_data.append(image_data)
        env.close()
        output_json_path = save_root_dir + ".json"
