...
def make_parser():
    ...
    # 009_SV_JPG.lmdb
    parser.add_argument(
        "--path", default="/media/camma-monitor/Storage_postprocessing2/pose_detection/mva/008_OT_JPG_iter2_mva_tracking_reid.lmdb", type=str, help="path to detections"
    )
    parser.add_argument(
...
def multi_imageflow_demo(preds_dict, frame_range, vis_folder, current_time, args, save_name, img_dir, using_mva=False):
    ...
    for frame_id in range(min_frame_id, max_frame_id + 1):
        ...
        if len(outputs):
            ...
            online_targets = tracker.update(outputs, assign_id=True)
            ...
            for t in online_targets:
                ...
                if tlwh[2] * tlwh[3] > args.min_box_area:
                    if using_mva:
                        online_results.append(tlbr.tolist() + [float(t.score), float(t.matching_score), int(tid)])
                    else:
                        online_results.append(tlbr.tolist() + [float(t.score), int(tid)])
...
def main(args):
    ...
    img_dir = os.path.join(args.root_dir, sub_dirs[0])
    img_names = os.listdir(img_dir)
    img_names = [x for x in img_names if x[-3:] == 'jpg' and x[0] == 'c']
    img_names.sort()
    
    base_name = args.path.rsplit(os.sep, 1)[1].rsplit('.', 1)[0]
    save_sub_dir = args.path.rsplit('/', 2)[1]
    tracking_dir = os.path.join(args.save_dir, save_sub_dir)
    os.makedirs(tracking_dir, exist_ok=True)
    tracking_path = os.path.join(tracking_dir, base_name + '_id.lmdb')
        ...
        for img_name in tqdm(img_names):
            ...
            for c_id, sub_dir in enumerate(sub_dirs):
                ...
                if boxes_data is not None and scores_data is not None:
                    ...
                    if len(boxes):
                        if using_mva:
                            boxes_scores = np.concatenate([np.copy(boxes), np.copy(scores)[:, None], np.copy(matching_scores)[:, None]], axis=1)
                        else:
    ...
    for c_id, sub_dir in enumerate(sub_dirs):
        # if c_id == 1 or c_id == 2:
        #     continue
        img_dir = os.path.join(args.root_dir, sub_dirs[c_id])
        final_dict = multi_imageflow_demo(preds_dicts[c_id], [min_frame_id, max_frame_id], vis_folder, current_time, args, str(c_id), img_dir, using_mva)
    ...
    for c_id, sub_dir in enumerate(sub_dirs):
        ...
        for f_id, preds in final_dict.items():
            ...
            if using_mva:
                matching_scores = boxes_scores[:, 5]
                tracking_ids = boxes_scores[:, 6].astype(int)
                tracking_txn.put(key_matching_scores, pickle.dumps(matching_scores))
                tracking_txn.put(key_tracking_ids, pickle.dumps(tracking_ids))
            else:
                tracking_ids = boxes_scores[:, 5].astype(int)
                tracking_txn.put(key_tracking_ids, pickle.dumps(tracking_ids))
