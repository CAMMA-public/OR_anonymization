...
def parse_args():
    ...
    parser.add_argument(
        "--image_shape", default=[1536, 2048], type=list, help="image shape"
    )
...
def inference(args):
    ...
    mva_path = os.path.join(mva_dir, base_name + '_mva_tracking.lmdb')
            ...
            for f_id in range(frame_num):
                ...
                for c_id in range(cam_num):
                    ...
                    if run_det_box and run_tracking_box:
                        sub_path = os.path.join('colorimage', img_dicts[c_id][f_id])
                        all_boxes = np.concatenate([boxes, tracking_boxes], axis=0)
                        all_scores = np.concatenate([scores, tracking_scores], axis=0)
                        all_output = model_forward(args, model, reid_model, device, sub_path, all_boxes, all_scores, height, width, c_id, dino)
                        if full_query:
                        ...
                        sub_path = os.path.join('colorimage', img_dicts[c_id][f_id])
                        all_output = model_forward(args, model, reid_model, device, sub_path, boxes, scores, height, width, c_id, dino)
                        all_features[c_id] = all_output
                    elif run_tracking_box:
                        all_tracking_output = model_forward(args, model, reid_model, device, sub_path, tracking_boxes, tracking_scores, height, width, c_id, dino)
                ...
                for c_id in range(cam_num):
                    ...
                    if len(anchor_feas):
                        ...
                        for sample_c_id in range(cam_num):
                            ...
                            matches_scores, matches = get_matching_dis(anchor_feas, sample_feas, reid_fea1=anchor_feas_reid, reid_fea2=sample_feas_reid, mode='l2', thresh=0.0, alpha=0.1, return_score=True)
